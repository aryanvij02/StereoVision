...
def stereo_depth_map(rectified_pair, variable_mapping):
    ...
    sbm.setNumDisparities(variable_mapping['NumofDisp']*16)
...
def save_load_map_settings(current_save, current_load, variable_mapping):
    ...
    if current_load != 0:
        ...
        cv2.setTrackbarPos("MinDisp", "Stereo", data['minDisparity'])
        cv2.setTrackbarPos("NumofDisp", "Stereo", data['numberOfDisparities'])
...
def create_trackbars() :
    ...
    cv2.createTrackbar("WindowSize", "Stereo", 15, 100, activateTrackbars)
...
if __name__ == '__main__':
    ...
    variable_mapping = {"SWS" : 115, "SpeckleSize" : 100, "SpeckleRange" : 15, "UniqRatio" : 10, "TxtrThrshld" : 100, "NumofDisp" : 1,
    "MinDisp": -25, "PreFiltCap" : 30, "PreFiltSize" : 105}
    # dict.fromkeys(variables)

    while True:
        left_image = cv2.imread('./testing/left.png', 0)

        right_image = cv2.imread('./testing/right.png', 0)

        # left_gray_frame = cv2.cvtColor(left_image, cv2.COLOR_BGR2GRAY)
        # right_gray_frame = cv2.cvtColor(right_image, cv2.COLOR_BGR2GRAY)
